<?php
    ...
    # SOLO PARA OPTION DE SELECT
        function SelectOption($dataArray, $indexReferen = 0, $textReferen = 1, $selectValue = "")
        {
            $select = "";
            if(is_array($dataArray["cuerpo"]) && count($dataArray["cuerpo"])>0)
            {
                for ($i = 0; $i < count($dataArray["cuerpo"]); $i++) {
                    $select .=      "<option value='".$dataArray["cuerpo"][$i][$indexReferen]."'";
                    if ($selectValue!= '' && $selectValue==$dataArray["cuerpo"][$i][$indexReferen])
                    {
                        $select .=  'selected class="sel"';
                    }
                    $select .=      ">".Mayusc($dataArray["cuerpo"][$i][$textReferen])."</option>";
                }
            }
            return $select;
        }
     ...
     */
        ...
        function InputTextPost($nameLabel,$nameInput,$placeholder="",$valueImput="",$classIcon="icon-text" , $classFieldset="c6" , $classInput = "")
        {
            $input ="";
            $input .='
                            <fieldset class="'.$classFieldset.'">
                                <label for="'.$nameInput.'" >'.$nameLabel.'</label>
                                <input  type="text" class="post '.$classInput.'" name="'.$nameInput.'" id="'.$nameInput.'" value="'.$valueImput.'" placeholder="'.$placeholder.'" >
                                <span class="post '.$classIcon.'"></span>
                            </fieldset>
            ';
            return $input;
        }
...
# FUNCIONES PARA FECHAS
    ...
    */
    function truncateFloat($number, $digitos)
    {
        ...
        return number_format($resultado, $digitos,'.', ''); # sin separador de millares
